...
export class TCDB extends Base {
  ...
  private getBrowserVersion(browserVersion: string) {
    ...
    this.actualBrowserVersion = `${browserVersion.split("/")[0]} ${
      browserVersion.split("/")[1].split(".")[0]
    }`;
  }
  ...
  private async initializeAttributes(browserVersion: string) {
    ...
    for (let attribute of [
      this.ATTRIBUTE_BROWSER_FREE_ENTRY,
      this.ATTRIBUTE_BROWSER_LIST,
      this.ATTRIBUTE_SOCIAL_CONVERSATION_INTERACTION_TYPE
    ]) {
      let options = {
        method: "GET",
      ...
      if (
        attribute === this.ATTRIBUTE_BROWSER_FREE_ENTRY ||
        attribute === this.ATTRIBUTE_BROWSER_LIST
      ) {
        ...
        this.attributeBrowserVersion.set(
          attribute,
          chromeBrowserVersionsSet.has(this.actualBrowserVersion)
            ? this.actualBrowserVersion
            : maxChromeVersion
        );
      }
    }
  ...
  public async submitSuccess(
    ...
    try {
      if (!this.build) {
        throw new Error("Cannot submit results without build.");
      }
      if (!this.token) {
        throw new Error("Cannot submit results without logging in.");
      }
      ...
      if (options && options.attributes && options.attributes.length) {
        for (let attribute of options.attributes) {
          if (
            (attribute.attribute &&
              attribute.attribute === this.ATTRIBUTE_BROWSER_FREE_ENTRY) ||
            attribute.attribute === this.ATTRIBUTE_BROWSER_LIST
          ) {
            ...
            execution.Attributes.push(
              socialConversationInteractionTypeAttribute
            );
          }
        }
      }
      ...
      this.log(
        `Successfully submitted success result for TC${tcNumber}.${tcVersion}`
      );
    } catch (error) {
...
export function tcdbTest(
  ...
  return test.each([[addStep(testName, jestDescription), testTrace(testName)]])(
    jestDescription,
    testFunction,
    timeoutMs
  );
