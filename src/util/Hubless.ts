...
export default class Hubless extends Base {
    ...
    async facebookDirectMessage(): Promise<XMLHttpRequest> {
        if (this.config.hubless === undefined) {
            this.log('Hubless is not configured');
            throw new Error('Hubless is not configured');
        }
        const request = new XMLHttpRequest();
        const url = `http://${this.config.hubless.server}:3000/facebook/message`;
        request.open('POST', url, false);
        request.timeout = 2500;
        request.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
        const postData = `{
    ...
    async twitterDirectMessage(): Promise<XMLHttpRequest> {
        if (this.config.hubless === undefined) {
            this.log('Hubless is not configured');
            throw new Error('Hubless is not configured');
        }
        const request = new XMLHttpRequest();
        const url = `http://${this.config.hubless.server}:3000/twitter/message`;
        request.open('POST', url, false);
        request.timeout = 2500;
        request.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
        const postData = `{
