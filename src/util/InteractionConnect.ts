...
export default class InteractionConnect extends Base {
  ...
  public async launch() {
    try {
      this.log(
        `Launching Interaction Connect at: ${
          this.config.interactionConnect.url
        }`
      );
      ...
      let serverTextInput = <puppeteer.ElementHandle>(
        (await this.page.waitFor(
          "input[data-inintest=ic-logon-server]"
        )).asElement()
      );
      await serverTextInput.type(this.config.ic.server);
      let chooseServerButton = <puppeteer.ElementHandle>(
        await this.page.$("button[data-inintest=ic-logon-choose-server]")
      );
      await chooseServerButton.click();

      this.log(
        `Logging on as: ${this.config.ic.user}, ${this.config.ic.password}`
      );
      let icAuthButton = <puppeteer.ElementHandle>(
        (await this.page.waitFor(
          'button[data-inintest="ic-logon-auth-type-Interaction Center Authentication"]'
        )).asElement()
      );
      await icAuthButton.click();
      let icAuthUsernameInput = <puppeteer.ElementHandle>(
        (await this.page.waitFor("input#username")).asElement()
      );
      await icAuthUsernameInput.type(this.config.ic.user);
      let icAuthPasswordInput = <puppeteer.ElementHandle>(
        await this.page.$("input#password")
      );
      await icAuthPasswordInput.type(this.config.ic.password);
      let icAuthSubmitButton = <puppeteer.ElementHandle>(
        await this.page.$("button[data-inintest=sts-ic-auth-submit]")
      );
      await icAuthSubmitButton.click();

      this.log("Changing station.");
      let changeStationButton = <puppeteer.ElementHandle>(
        (await this.page.waitFor(
          "button[data-inintest=ic-change-station-submit]"
        )).asElement()
      );
      ...
      await this.page.waitFor(
        "button[data-inintest=inin-command-button-pickup-base]"
      );
  ...
  private async checkOrWaitFor(
    ...
    return <puppeteer.ElementHandle>(
      (await this.page.waitFor(selector, { timeout: timeout })).asElement()
    );
  }
  ...
  private async getInteractionStates() {
    ...
    const elements = <Array<puppeteer.ElementHandle>>(
      await this.page.$$(
        "div.queue-content div[col-container-name=\"'body'\"] div.filtered-interaction-queue-row"
      )
    );
    for (const element of elements) {
      let result = await this.page.evaluate(row => {
        ...
        const interaction = row
          .getAttribute("data-inintest")
          .split("-")
          .slice(-1)[0];
        const state = row
          .querySelector('div[data-inintest="qcol-State"] span')
  ...
  public async getInteractionRow(interaction: string) {
    ...
    const results = await this.page.$$(
      `div.queue-content div[data-inintest*="${interaction}"]`
    );
    if (results.length > 0) {
      return results[1];
    } else {
  ...
  private async waitForStateAndGetInteractionInState(
    state: string,
    ...
    await this.checkOrWaitFor(
      `div.queue-content div[data-inintest="qcol-State"] span[uib-tooltip*="${state}"]`,
      timeout
    );
  ...
  private async performActionOnInteraction(
    interaction: string,
    action: string
  ) {
    await this.clickOnInteraction(interaction);
    await this.page.click(
      `button[data-inintest=inin-command-button-${action}-base]`
    );
    this.log(`Performed '${action}' on interaction ${interaction}`);
  }

  public async pickupAlertingInteraction(
    timeout: number = this.DEFAULT_TIMEOUT
  ) {
    await this.page.bringToFront();
    const interactionToPickUp = await this.waitForStateAndGetInteractionInState(
      "Alerting",
      undefined,
      timeout
    );
  ...
  public async pickupInteraction(
    interaction: string,
    timeout: number = this.DEFAULT_TIMEOUT
  ) {
    await this.page.bringToFront();
    ...
    const pickedupInteraction = await this.waitForStateAndGetInteractionInState(
      "ACD - Assigned",
      interaction,
      timeout
    );

    return pickedupInteraction === interaction
      ? pickedupInteraction
      : undefined;
  }

  public async disconnectInteraction(
    interaction: string,
    timeout: number = this.DEFAULT_TIMEOUT
  ) {
    await this.page.bringToFront();
    ...
    const disconnectedInteraction = await this.waitForStateAndGetInteractionInState(
      "Disconnected",
      interaction,
      timeout
    );

    return disconnectedInteraction === interaction
      ? disconnectedInteraction
      : undefined;
  }
  ...
  public async holdInteraction(
    ...
    const heldInteraction = await this.waitForStateAndGetInteractionInState(
      "Held",
      interaction,
      timeout
    );
  ...
  public async replyToFacebookRootPostAndVerifyReply(reply: string) {
    await this.page.bringToFront();
    const replyTextArea = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `textarea[data-inintest="social-conversation-composition-text-input"]`
      )
    );
    ...
    await this.page.click(
      'button[data-inintest="social-conversation-composition-send-link"]'
    );
  ...
  public async replyToFacebookCommentAndVerifyReply(
    comment: puppeteer.ElementHandle,
    reply: string
  ) {
    await this.page.bringToFront();
    let replyButton = <puppeteer.ElementHandle>(
      await comment.$("span.toolbar-image")
    );
    await replyButton.click();

    const replyTextArea = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `div.comment-window textarea[data-inintest="social-conversation-composition-text-input"]`
      )
    );
    await replyTextArea.type(reply);
    ...
    await this.page.click(
      'div.comment-window button[data-inintest="social-conversation-composition-send-link"]'
    );
  ...
  public async verifyFacebookPostVisible(post: string) {
    await this.page.bringToFront();
    const postDiv = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(`div.company-post-text`)
    );
  ...
  public async openTab(tabName: string) {
    await this.page.bringToFront();
    await this.page.click(
      `span.inin-docking-region-display-name[uib-tooltip="${tabName}"]`
    );
  ...
  public async clearQueueFilter() {
    await this.page.bringToFront();
    const iconFilter = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(`i.icon-filter`, this.DEFAULT_TIMEOUT, true)
    );
    await iconFilter.click();
    const interactionTypeSelect = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `div.popover-content div[data-inintest="filter-interaction-types-multiselect inin-checkbox-multiselect-All"]`
      )
    );
    await interactionTypeSelect.click();
    const deselectAllButton = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `div.popover-content button[data-inintest="inin-checkbox-multiselect-deselect-all"]`
      )
    );
  ...
  public async filterQueueOnSocialConversations() {
    ...
    const interactionTypeSelect = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `div.popover-content div[inin-checkbox-multiselect-options="interactionTypes"] button`
      )
    );
    await interactionTypeSelect.click();
    const socialConversationCheckbox = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `div.popover-content input[data-inintest="inin-checkbox-multiselect-item-checkbox-Social Conversation"]`
      )
    );
  ...
  private async openSettings() {
    const userMenuButton = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `button[data-inintest="user-menu-dropdown-toggle"]`,
        this.DEFAULT_TIMEOUT,
        true
      )
    );
    await userMenuButton.click();
    const settingsButton = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `button[data-inintest="navbar-configuration-link"]`
      )
    );
    await settingsButton.click();
    return <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(`div[data-inintest="configuration-modal"]`)
    );
  }
  ...
  public async openRingSoundsSettings() {
    ...
    const ringSoundsLink = <puppeteer.ElementHandle>(
      await this.checkOrWaitFor(
        `a[data-inintest="configuration-modal-menu-item-ringSounds"]`
      )
    );
  ...
  public async getCurrentSocialConversationRingSound() {
    await this.page.bringToFront();
    await this.checkOrWaitFor(
      `ic-ring-sound-select#ring-sounds-form-social-conversation`
    );
    return await (await (await this.page.$(
      "select#ic-ring-sound-select-ring-sounds-form-social-conversation"
    )).getProperty("value")).jsonValue();
  }
  ...
  public async toggleSocialConversationRingSound() {
    ...
    if (currentRingSound === this.originalRingSound) {
      ...
      for (let option of await this.page.$$(
        "select#ic-ring-sound-select-ring-sounds-form-social-conversation option"
      )) {
        let optionText = (await (await option.getProperty(
          "value"
        )).jsonValue()) as string;
