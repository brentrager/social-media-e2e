...
export default class Facebook extends Base {
  ...
  public async launch() {
    try {
      ...
      this.log(
        `Logging on as: ${this.config.facebook.user}, ${
          this.config.facebook.password
        }`
      );
      let facebookUsernameInput = <puppeteer.ElementHandle>(
        (await this.page.waitFor("input#email")).asElement()
      );
      await facebookUsernameInput.type(this.config.facebook.user);
      let facebookPasswordInput = <puppeteer.ElementHandle>(
        await this.page.$("input#pass")
      );
      await facebookPasswordInput.type(this.config.facebook.password);
      let facebookSubmitButton = <puppeteer.ElementHandle>(
        await this.page.$('input[value="Log In"]')
      );
  ...
  public async postRandom() {
    ...
    try {
      ...
      try {
        facebookWriteSomethingTextArea = <puppeteer.ElementHandle>(
          (await this.page.waitFor('div[role="textbox"]', {
            timeout: 5000
          })).asElement()
        );
      } catch (error) {
        try {
          facebookWriteSomethingTextArea = <puppeteer.ElementHandle>(
            (await this.page.waitFor('div[contenteditable="true"]', {
              timeout: 5000
            })).asElement()
          );
        } catch (error) {
          facebookWriteSomethingTextArea = <puppeteer.ElementHandle>(
            (await this.page.waitFor("textarea", { timeout: 5000 })).asElement()
          );
        }
      }
      await facebookWriteSomethingTextArea.focus();
      await facebookWriteSomethingTextArea.click();
      let facebookPageWriteSomethingInput = <puppeteer.ElementHandle>(
        (await this.page.waitFor("div[contenteditable=true]")).asElement()
      );
